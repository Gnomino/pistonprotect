...
public class PistonListener implements Listener {
	...
	private WorldGuardPlugin getWG() {
		return (WorldGuardPlugin) pl.getServer().getPluginManager()
	...
	public void onPistonExtend(BlockPistonExtendEvent event) {
		...
		RegionManager regionManager = wg.getRegionManager(event.getBlock()
				.getWorld());
		ApplicableRegionSet pistonRegions = regionManager
		...
		for (Block b : (event.getBlocks())) {
			ApplicableRegionSet regions = regionManager.getApplicableRegions(b
					.getLocation());
			if (regions.size() > 0) {
	...
	public void onPistonRetract(BlockPistonRetractEvent event) {
		if (event.isSticky()) {
			...
			if (pulled.getPistonMoveReaction() == PistonMoveReaction.MOVE
					|| pulled.getPistonMoveReaction() == PistonMoveReaction.BREAK) {
				WorldGuardPlugin wg = getWG();
				if (wg == null) {
					return;
				}
				RegionManager regionManager = wg.getRegionManager(piston
						.getWorld());
				ApplicableRegionSet pistonRegions = regionManager
						.getApplicableRegions(piston.getLocation());
				ArrayList<String> pistonRegionsIDs = new ArrayList<String>();
				for (ProtectedRegion r : pistonRegions) {
					pistonRegionsIDs.add(r.getId());
				}
				for (ProtectedRegion r : regionManager
						.getApplicableRegions(pulled.getLocation())) {
					if (!pistonRegionsIDs.contains(r.getId())) {
						event.setCancelled(true);
						if (pl.getConfig().getBoolean("log_fails")) {
							pl.getLogger().info(
									"Blocked sticky piston retracting at "
											+ event.getBlock().getX()
											+ ";"
											+ event.getBlock().getY()
											+ ";"
